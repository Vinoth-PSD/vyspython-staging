    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            if edu_level == '1' or edu_level == '2':
                #print('inside',field_of_study)
                field_ofstudy =  models.Profileedu_degree.objects.filter(is_deleted=0,fieldof_study=field_of_study)
            else:
                 field_ofstudy =  models.Profileedu_degree.objects.filter(is_deleted=0,edu_level=edu_level,fieldof_study=field_of_study)
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            prof_details= {
                            ...
                            "completion_per":calculate_profile_completion(profile_id),
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if get_limits is True: 
                ...
                return JsonResponse({"Status": 0, "message": "No access to bookmark the profile"}, status=HTTP_200_OK)
...
def can_send_express_interest(profile_id):
    ...
    plan = models.PlanFeatureLimit.objects.filter(plan_id=plan_id).first()
    ...
    if plan and plan.express_int_count is not None:
        # print('123456')
        # print('datetime',datetime.today())
        if plan.express_int_count == 0:
            return False  # Not allowed to send express interests
        elif plan.express_int_count == 1:
